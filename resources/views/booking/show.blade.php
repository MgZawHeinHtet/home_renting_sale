<x-layout>
            ...
            <div class="rounded-lg overflow-hidden my-5">
                <div class=" flex flex-col lg:flex-row">
                    ...
                    <div class="flex-1 border-[1px] p-3 text-[14px]">
                        ...
                        <div class="pt-3">
                            <p class="">Price</p>
                            <p class="font-bold">
                                {{ round($booking->total_days / 30) }}month{{ $days > 0 ? ',' . $days . 'days' : '' }}
                            </p>
                ...
                <div class=" border-[1px] flex flex-col lg:flex-row gap-5 lg:gap-0 p-5 text-[14px]">
                    <div class="w-full lg:w-1/3 text-center">
                        ...
                        <p class=" pb-3 text-12px">For 1 week before check_in date</p>
                        <p class="">To cancel free, you must cancel <span class="font-bold">before </span>
                            {{ date('M d,Y h:m', strtotime($booking->cancellable_date)) }}
                            Yangon time. After that time, you will have to pay half of price to cancel.</p>
                    </div>
                        ...
                        <div
                            ...
                            <form action="/booking/{{ $booking->id }}/cancel/step1" method="GET">
                                @if (date('Y-m-d') > date('Y-m-d', strtotime($booking->cancellable_date)))
                                    <button disabled
                                        class="bg-orange-600 opacity-45 hover:bg-orange-700 text-white rounded-sm px-2 py-1 text-sm text-nowrap">
    ...
    <div
        class="hidden review-parent w-screen h-screen bg-black bg-opacity-20 z-50 fixed top-0 right-0  justify-center items-center">
        <div class="w-[500px] flex flex-col  items-center p-6 h-60 bg-white shadow-lg z-50 rounded-lg relative">
            <button class="review-btn absolute top-2 right-2">❌</button>

            <p class="text-center font-bold  capitalize mb-3"> Review this Property 😁
            <form action="/properties/{{ $booking->property->id }}/review" method="POST">
                @csrf
                <textarea placeholder="write something" name="review"
                    class="border-2 outline-none w-full p-1 rounded border-slate-200" id="" cols="30" rows="5"></textarea>
                <x-error name="review"></x-error>
                <button class="px-6 text-white w-full py-2 bg-yellow-500">send</button>
            </form>
        </div>
    </div>
    <div
        class="hidden rating-parent w-screen h-screen bg-black bg-opacity-20 z-50 fixed top-0 right-0  justify-center items-center">
            ...
            <form action="/properties/{{ $booking->property->id }}/rating" method="POST" onchange="submit()">
                ...
                <div class="p-4 rounded-lg mt-4">
                    <p class="">Total Rating -
                        {{ $booking->property->avg_rating }}/{{ $booking->property->ratedUsers->count() }}</p>
                </div>
            </form>
        </div>
    </div>
    ...
    <script>
        ...
        const reviewParent = document.querySelector('.review-parent');
        const reviewBtn = document.querySelector('.review-btn')
        const openRatingBtn = document.querySelector('.open-btn')
        const openReviewBtn = document.querySelector('.review-open-btn')
        ...
        openReviewBtn.addEventListener('click', function() {
            ...
            reviewParent.classList.remove('hidden')
            reviewParent.classList.add('flex')
        })
